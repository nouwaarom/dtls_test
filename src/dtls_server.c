...
int dtls_main(void) {
    ...
    //char* shared_identity = "Identity";
    //char* shared_secret = "ThisIsAVerySecretSecret";
    //mbedtls_ssl_conf_psk(&conf, shared_secret, strlen(shared_secret), shared_identity, strlen(shared_identity));
    ...
    ret = mbedtls_x509_crt_parse(&srvcert, (const unsigned char*) test_certificate, strlen(test_certificate) + 1);
    if (ret != 0) {
    ...
    //ret = mbedtls_x509_crt_parse( &srvcert, (const unsigned char *) test_private_key, strlen(test_private_key)+1);
    //if( ret != 0 )
    //{
    //    printf( " failed\n  !  mbedtls_x509_crt_parse returned -0x%x\n\n", (unsigned int) -ret );
    //    goto exit;
    ...
    ret = mbedtls_pk_parse_key(&pkey, (const unsigned char*) test_private_key, strlen(test_private_key) + 1, NULL, 0);
    if (ret != 0) {
